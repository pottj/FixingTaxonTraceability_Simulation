SimulationFunction = function(number_taxa, number_quads, repeats, data1, data2, verbose=F,FFT_only_if_PhyloDec=F){
  # number_taxa = 6
  # number_quads = 5
  # repeats = 100
  # data1 = copy(test1)
  ...
  # Step 3: get number of all combinations 
  nr_allCombis = choose(m,k)
  if(nr_allCombis<repeats){
    message("       Testing all combination only once ...")
    repeats = nr_allCombis 
  }else{
  ...
  inputs = data.table(rep = 1:repeats,
                      sampling = "NA")
  set.seed(2015)
  ...
  dumTab = foreach(i = 1:repeats)%do%{
    ...
    # Step 5.2: FFTs
    time0 = Sys.time()
    if(FFT_only_if_PhyloDec==F | FFT_only_if_PhyloDec==T & test_1_res == "PhyloDec"){
